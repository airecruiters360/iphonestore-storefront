...
module.exports = async function handler(req, res) {
  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    // The Supabase table doesn't have dedicated columns for the shipping
    // address or extended warranty yet, so fold them into the free-text
    // "message" field rather than risk an insert failure by referencing
    // columns that may not exist. The full details always live natively
    // on the Stripe PaymentIntent (shipping object + metadata) regardless.
    const shippingLine = [
      metadata.shipping_address1,
      metadata.shipping_address2,
    ...
    const extraNotes = [
      ...
      metadata.warranty_label ? `Warranty: ${metadata.warranty_label}` : null,
    ]
      .filter(Boolean)
      .join(" | ");
    const combinedMessage = [metadata.message, extraNotes].filter(Boolean).join(" || ") || null;
  ...
  // Financing-abandonment tracking (scoping doc, section 8) can hook in
  // here later via payment_intent.payment_failed / .canceled events.
